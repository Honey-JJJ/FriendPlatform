...
class manager
{
	...
	$sql="select * from appointment,user where appointment.uno = user.uno";
	$result = mysqli_query($this->link,$sql);
 
echo '<table style="margin: 15vh auto 0;" border="0"><td>约会地点</td><td>日期</td><td>状态</td><td>修改</td></tr>';
    while($row = mysqli_fetch_array($result))
    {
    echo "<tr><td>{$row['place']} </td> "."<td>{$row['date']} </td> "."<td>{$row['status']} </td> ";
	?>
	<td>
